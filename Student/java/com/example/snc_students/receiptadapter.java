...
public class receiptadapter extends FirebaseRecyclerAdapter<receiptupload, receiptadapter.viewholder> {
    ...
    Bitmap bitmap;
    Context c;
    public receiptadapter(@NonNull FirebaseRecyclerOptions<receiptupload> options, ProgressBar pbar, Activity activity) {
        super(options);
        this.pbar = pbar;
        ...
        if(!isNetworkAvailable()){
            ...
            FirebaseDatabase.getInstance().getReference().child("receipt").orderByChild("stu_id").equalTo(uid)
                    .addListenerForSingleValueEvent(new ValueEventListener() {
                        @Override
                        public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                            //Toast.makeText(activity,""+dataSnapshot.getChildrenCount(),Toast.LENGTH_SHORT).show();
